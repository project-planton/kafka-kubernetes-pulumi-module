...
func Resources(ctx *pulumi.Context, input *Input) error {
	...
	ingressHostname := hostname.GetExternalKowlDashboardHostname(input.KafkaKubernetesId, input.EnvironmentName, input.KafkaIngressDomain)
	virtualServiceObject := buildVirtualServiceObject(fmt.Sprintf("%s-public", KubeServiceName),
		KubeServiceName, input.NamespaceName, ingressHostname,
		input.EnvironmentName, input.KafkaIngressDomain)
	if err := addVirtualService(ctx, virtualServiceObject, svc, input.WorkspaceDir); err != nil {
		return errors.Wrap(err, "failed to add public virtual service")
	}
	return nil
}
...
func addService(ctx *pulumi.Context, input *Input) (*corev1.Service, error) {
	svc, err := corev1.NewService(ctx, input.KowlDeploymentName, &corev1.ServiceArgs{
		...
		Spec: &corev1.ServiceSpecArgs{
			...
			Selector: pulumi.StringMap{
				englishword.EnglishWord_app.String(): pulumi.String(input.KowlDeploymentName),
			},
...
spec:
	...
	- web
	hosts:
	- kc-planton-pcs-prod-mon-kowl.dev.example.com
	http:
	- name: kowl
	  route:
	  - destination:
	      host: kowl.tenant-product-kafka-dev-main.svc.cluster.local
	      port:
	        number: 80
*/
...
func getGatewayName(productEnvName, domain string) string {
	envDomain := fmt.Sprintf("%s.%s", productEnvName, domain)

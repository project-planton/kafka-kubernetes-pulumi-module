...
type Input struct {
	...
	KowlConfigMap      *corev1.ConfigMap
}
...
func addDeployment(ctx *pulumi.Context, input *Input) error {
	labels := input.Labels
	labels[englishword.EnglishWord_app.String()] = input.KowlDeploymentName
	_, err := appsv1.NewDeployment(ctx, input.KowlDeploymentName, &appsv1.DeploymentArgs{
		Metadata: &metav1.ObjectMetaArgs{
			Name:      pulumi.String(input.KowlDeploymentName),
					...
					Volumes: corev1.VolumeArray{
						corev1.VolumeArgs{
							ConfigMap: corev1.ConfigMapVolumeSourceArgs{
								Name: input.KowlConfigMap.Metadata.Name(),
							},
	...
	}, pulumi.IgnoreChanges([]string{"metadata", "status"}), pulumi.Parent(input.KowlConfigMap))
	if err != nil {
		return errors.Wrap(err, "failed to add deployment")
	}
	return nil
